...
import com.mysmartshop.cart.cartItemRepository.CartItemRepository;
...
public class CartServiceImpl implements CartService{
	...
	public List<CartItem> addToCart(String productId) {
		...
		if(!checkItem.isPresent()) {
			...
			Product prod = fetchProduct(productId);
			
			CartItem item = new CartItem();
			item.setProductId(productId);
			item.setQuantity(1);
			item.setTotalPrice(prod.getPrice());
	...
	public List<CartItem> updateQuantity(String productId, int quantity) {
		...
		if(checkItem.isPresent()) {
			...
			item.setTotalPrice((item.getTotalPrice()/(quantity-1))*quantity);
			itemRepository.save(item);
		}
